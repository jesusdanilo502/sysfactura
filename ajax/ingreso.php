...
switch($_GET["op"]){
    case 'guardaryeditar':
        if (empty($idcategor)) {
            $answer= $ingreso->insertar($idproveedor,$idusuario,$tipo_comprobante,$serie_comprobante,$num_comprobante,$fecha_hora,$impuesto,$total_compra,$_POST["idarticulo"],$_POST["cantidad"],$_POST["precio_compra"],$_POST["precio_venta"]);
            echo $answer ? "Ingreso registrado" : "No se pudieron registrar todos los datos del ingreso";
    ...
    case 'anular':
        $answer=$ingreso->anular($idingreso);
        echo $answer ? "Ingreso anulado" : "Ingreso no se puede anular";
        break;

    case 'mostrar':
        $answer=$ingreso->mostrar($idingreso);
    ...
    case 'listar':
        ...
        while ($reg=$answer->fetch_object()){
            $data[]=array(
                ...
                "6"=>'$ '.number_format($reg->total_compra),
        ...
        while ($reg=$answer->fetch_object()){
            $data[]=array(
                "0"=>'<button class="btn btn-warning"
                 onclick="agregarDetalle('.$reg->idarticulo.',\''.$reg->nombre.'\')"><span class="fa fa-plus"></span></button>',
                "1"=>$reg->nombre,
                "2"=>$reg->categoria,
                "3"=>$reg->codigo,
                "4"=>$reg->stock,
